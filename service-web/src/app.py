...
hostname = os.getenv("HOSTNAME", default = "service_web")
SERVICE_HOST = os.getenv("SERVICE_HOST", default = "localhost")
SERVICE_PORT = os.getenv("SERVICE_PORT", default = "8001")
...
def name():
  ...
  <h3>Hostname: {0}</h3>
  """.format(hostname)
  logger.info('[SERVICE-WEB]', extra={'hostname': hostname, 'method': request.method, 'url': request.url, 'user-agent': request.user_agent})
...
def get():
  ...
  logger.info('[SERVICE-WEB]', extra={'hostname': hostname, 'status': resp.status_code, 'url': resp.url, 'data': resp.text, 'content-type': resp.headers['content-type']})
